import { ApiError, apiFetch, registerRefreshHandler } from './api-client';
...
describe('apiFetch', () => {
  ...
  it('returns the body on a successful call without touching the refresh handler', async () => {
    ...
    const result = await apiFetch<{ ok: boolean }>('/v1/users/me', 'token-1');
  ...
  it('throws the original 401 when no refresh handler is registered', async () => {
    ...
    await expect(apiFetch('/v1/users/me', 'token-1')).rejects.toThrow(ApiError);
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
